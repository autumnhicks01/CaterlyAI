// Test script to demonstrate caching and performance improvements (CommonJS version)

// Dynamically import ESM module
async function runTest() {
  try {
    // We need to use dynamic import for ESM modules in CommonJS
    const outreachAgent = await import('./agents/outreachAgent.js');
    const { generateDripCampaign, templateCache } = outreachAgent;
