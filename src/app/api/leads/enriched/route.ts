...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`Fetching enriched leads for user: ${userId}`);
    
    // Fetch only enriched leads for the user
    const { data, error } = await supabase
      .from('saved_leads')
      .select('*')
      .eq('user_id', userId) // Filter by user_id to only show the user's leads
      .in('status', ['enriched', 'saved']) // Include both enriched and saved leads
      .order('updated_at', { ascending: false });
    
    if (error) {
      console.error('Error fetching enriched leads:', error);
      return Response.json(
        { error: 'Failed to fetch enriched leads: ' + error.message },
        { status: 500 }
      );
    }
    
    console.log(`Found ${data?.length || 0} enriched leads for user ${userId}`);
    ...
    const processedLeads = data.map(lead => {
      ...
      return {
        ...lead,
        lead_score: lead.lead_score || leadScore,
        lead_score_label: lead.lead_score_label || leadScoreLabel
      };
    });
