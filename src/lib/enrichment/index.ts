...
// This module provides a simple interface for the enrichment process with a URL
...
export async function processUrl(url: string): Promise<{ jobId: string }> {
  ...
  try {
    const response = await fetch("/api/enrichment/url", {
      method: "POST",
      headers: {
...
export async function getJobStatus(jobId: string): Promise<EnrichmentJob> {
  ...
  try {
    const response = await fetch(`/api/enrichment/status/${jobId}`);
      ...
      try {
        ...
        if (data.status === "complete") {
          ...
          if (data.result) {
            ...
            reject(new Error("Job completed but no result was returned"));
          }
          return;
        }
